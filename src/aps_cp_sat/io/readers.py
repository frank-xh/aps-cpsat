...
def load_grade_catalog(steel_info_path: Path) -> MergedGradeRuleCatalog:
    return MergedGradeRuleCatalog.build(steel_info_path)


def load_orders(orders_path: Path, steel_info_path: Path, cfg: PlannerConfig, catalog: MergedGradeRuleCatalog) -> pd.DataFrame:
