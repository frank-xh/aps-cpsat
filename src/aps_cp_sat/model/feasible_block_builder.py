...
def build_seed_buckets(
    ...
    for _, row in orders_df.iterrows():
        ...
        # Determine candidate lines. Dual/either orders must be exposed to both
        # production lines; otherwise DirectionalBlockGen can collapse to one
        # line even when big_roll templates and data are healthy.
        if cap in {"big_only", "large"}:
            candidate_lines = ["big_roll"]
        elif cap in {"small_only", "small"}:
            candidate_lines = ["small_roll"]
        else:
...
def _grow_block_from_seed_directional(
    ...
    candidate_ton_min = float(getattr(cfg.model, "block_generator_candidate_tons_min", 200.0))
...
def _compute_block_profile(
    ...
    return {
        ...
        "direct_edge_count": sum(
            1 for i in range(len(order_ids) - 1) if graph.edge_type(order_ids[i], order_ids[i + 1]) == "DIRECT_EDGE"),
        "real_bridge_edge_count": sum(
            1 for i in range(len(order_ids) - 1) if graph.has_real_bridge(order_ids[i], order_ids[i + 1])),
        "virtual_family_edge_count": sum(
            1 for i in range(len(order_ids) - 1) if graph.has_guarded_family(order_ids[i], order_ids[i + 1])),
    }
...
def generate_candidate_macro_blocks_directional(
    ...
    # Fair line interleaving: avoid starving big_roll when one line dominates
    # insertion order or the time budget is tight.
    buckets_by_line: Dict[str, List[tuple[str, List[str]]]] = {"big_roll": [], "small_roll": []}
    for bucket_key, seed_oids in buckets.items():
        parts = bucket_key.split("|")
        bucket_line = parts[0] if parts else "big_roll"
        buckets_by_line.setdefault(bucket_line, []).append((bucket_key, seed_oids))

    ordered_bucket_items: List[tuple[str, List[str]]] = []
    max_len = max(len(v) for v in buckets_by_line.values()) if buckets_by_line else 0
    for i in range(max_len):
        for bucket_line in ("big_roll", "small_roll"):
            line_items = buckets_by_line.get(bucket_line, [])
            if i < len(line_items):
                ordered_bucket_items.append(line_items[i])

    if not ordered_bucket_items:
        ordered_bucket_items = list(buckets.items())

    for bucket_key, seed_oids in ordered_bucket_items:
    ...
    # Recovery pass: if one line collapsed to zero blocks despite healthy buckets,
    # run a targeted second pass for that line only. This protects against
    # line starvation under tight time budgets.
    if not any(b.line == "big_roll" for b in blocks):
        big_roll_buckets = [item for item in ordered_bucket_items if str(item[0]).startswith("big_roll|")]
        recovery_budget = min(max(40, target_blocks // 6), 120)
        for bucket_key, seed_oids in big_roll_buckets:
            if len([b for b in blocks if b.line == "big_roll"]) >= recovery_budget:
                break
            parts = bucket_key.split("|")
            line = parts[0] if parts else "big_roll"
            seeds_to_use = list(seed_oids)[: max_seed_per_bucket * 2]
            for seed_oid in seeds_to_use:
                for mode in ("direct_friendly", "real_bridge_friendly"):
                    sequences = _grow_block_from_seed_directional(
                        seed_oid=seed_oid,
                        line=line,
                        graph=graph,
                    ...
                    for seq in sequences:
                        ...
                        block = MacroBlock(
                            ...
                            bridge_dependency_score=bridge_dep,
                            source_bucket_key=bucket_key,
                            source_generation_mode=f"{mode}_recovery",
                        )
                        blocks.append(block)
                        stats.unique_orders_covered.update(seq)
                        if len([b for b in blocks if b.line == "big_roll"]) >= recovery_budget:
    ...
    for line in lines:
        ...
        for pass_idx in range(3):
            ...
            for seed_oid in seed_orders:
                ...
                for seq, cost in _randomized_dfs_walk(graph, seed_oid, line, cfg, pass_rng, max_depth=30,
                                                      direction="forward"):
                    seq_tuple = tuple(seq)
                    ...
                    stats.blocks_by_ton_range[blocks_by_ton_range] = stats.blocks_by_ton_range.get(blocks_by_ton_range,
                                                                                                   0) + 1
